...
# ------------------------------------------ methods ------------------------------------------
# ---------------------------------------------------------------------------------------------
def fluidOverview(df, particleDf, lims):
...
#     fig, axes = plt.subplots(figsize = (5,2.5))
    ...
    # axes[0].plot(particleDf.position_x, particleDf.position_y, color='magenta', alpha = 0.5, linewidth=2, zorder=4)  # Trajectory line
...
#     axes[1].set_ylabel("$y ~ (\\mathrm{mm})$")
    ...
    myColor='#F7F7F7'
    mask = Circle((positionX, positionY), squirmerRadius, transform=axes[0].transData, color=myColor, linestyle='', alpha=1, zorder=2)
    axes[0].add_patch(mask)
    mask = Circle((positionX, positionY), squirmerRadius, transform=axes[1].transData, color=myColor, linestyle='', alpha=1, zorder=2)
    axes[1].add_patch(mask)

    axes[0].plot(particleDf.position_x[:-2], particleDf.position_y[:-2], color='magenta', alpha = 0.5, linewidth=1, zorder=4)  # Trajectory line
    axes[1].plot(particleDf.position_x[:-2], particleDf.position_y[:-2], color='magenta', alpha = 0.5, linewidth=1, zorder=4)  # Trajectory line

    
    axes[0].arrow(
        particleDf.position_x.values[-2], particleDf.position_y.values[-2],
        positionX - particleDf.position_x.values[-2], positionY - particleDf.position_y.values[-2],
...
def pltFluidVelocity(df, particleDf, lims):
#     df = df.query('coordinate_y <= 10 & coordinate_y >= -10')
    # figure preparation
#     fig, axes = plt.subplots(figsize = (5,2.5))
    ...
    positionX = particleDf.position_x.values[-1]
    positionY = particleDf.position_y.values[-1]
    # axes.plot(particleDf.position_x, particleDf.position_y, color='magenta', alpha = 0.5, linewidth=2, zorder=4)  # Trajectory line
...
#     cbar.set_ticklabels(['$0$','$1.5$','$3$'])
    ...
    # Create a mask as a filled polygon (circle patch with alpha for masking)
    myColor='#F7F7F7'
    mask = Circle((positionX, positionY), squirmerRadius, transform=axes.transData, color=myColor, linestyle='', alpha=1, zorder=2)
    axes.add_patch(mask)

    axes.plot(particleDf.position_x[:-2], particleDf.position_y[:-2], color='magenta', alpha = 0.5, linewidth=1, zorder=4)  # Trajectory line
